    ...
    <div className={`min-h-screen bg-stone-100 text-stone-900 ${fontClass}`} dir={dir}>
      ...
      <header className="max-w-6xl mx-auto px-6 pt-6 pb-20 grid md:grid-cols-2 gap-12 items-center">
        <div>
          ...
          <h1 className="font-display font-bold text-5xl md:text-6xl leading-[1.05] mt-5">
            {t.hero.title1}
          ...
          <div className="flex flex-wrap gap-4 mt-8">
            <a href="#shop" className="inline-flex items-center gap-2 bg-orange-700 text-stone-50 px-6 py-3 font-mono2 text-sm hover:bg-orange-800 transition">
              <ShoppingBag size={16} /> {t.hero.ctaShop}
            </a>
            <a href="#workshops" className="inline-flex items-center gap-2 border border-stone-900 px-6 py-3 font-mono2 text-sm hover:bg-stone-900 hover:text-stone-50 transition">
        ...
        <div className="relative h-[420px] hidden md:block">
          <div className="absolute top-0 right-0 w-64 h-80 bg-gradient-to-br from-orange-700 via-amber-600 to-indigo-900 shadow-xl rotate-3" />
          <div className="absolute bottom-0 left-0 w-56 h-72 bg-gradient-to-br from-indigo-900 via-indigo-800 to-stone-900 shadow-xl -rotate-6" />
          <div className="absolute top-24 left-16 w-40 h-40 bg-gradient-to-br from-amber-400 to-orange-600 shadow-lg rotate-12" />
        </div>
      </header>
      ...
      {/* WORKSHOPS */}
      <section id="workshops" className="bg-indigo-950 text-stone-50">
      ...
      <section className="max-w-6xl mx-auto px-6 py-20">
        ...
        <div className="grid md:grid-cols-3 gap-6 mt-8">
          {t.testimonials.map((tm) => (
            <div key={tm.name} className="bg-white border border-stone-200 p-6">
              <Quote size={20} className="text-orange-700 mb-3" />
              <p className="text-stone-600 text-[0.95rem] leading-relaxed">{tm.quote}</p>
              <div className="mt-4 text-sm font-mono2 text-stone-500">{tm.name} · {tm.place}</div>
            </div>
          ))}
        </div>
      </section>
              ...
              <>
                ...
                <div className="flex gap-3 mt-2">
                  ...
                  <button
                    ...
                    className="bg-amber-500 text-stone-900 px-5 py-3 font-mono2 text-sm hover:bg-amber-400 transition"
                  >
                    {t.contact.button}
                  </button>
                </div>
