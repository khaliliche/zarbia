...
export default function Nav() {
  ...
  const navLink =
    "relative py-1 hover:text-stone-900 transition after:content-[''] after:absolute after:left-0 after:-bottom-0.5 after:h-px after:w-0 after:bg-orange-700 after:transition-all hover:after:w-full";
    ...
    <>
      <nav className="sticky top-0 z-50 bg-stone-100/85 backdrop-blur-md border-b border-stone-200/80">
        <div className="max-w-6xl mx-auto flex items-center justify-between px-6 py-5">
          <Link
            href="/"
            className="flex items-center gap-2.5 font-display font-bold text-xl tracking-tight group"
          >
          ...
          <div className="hidden md:flex gap-8 text-sm text-stone-600">
            <Link href="/rugs" className={navLink}>
              {t.nav.rugs}
            </Link>
            <Link href="/djellabas" className={navLink}>
              {t.nav.djellabas}
            </Link>
